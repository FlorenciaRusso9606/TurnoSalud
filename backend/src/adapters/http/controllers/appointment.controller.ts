...
const bookUseCase = new BookAppointmentUseCase(
  appointmentRepo,
  availabilityRepo,
);
...
const getAvailableUseCase = new GetAvailableSlotsUseCase(
  availabilityRepo,
  appointmentRepo,
);
...
export const getAdminAppointments = async (req: Request, res: Response) => {
  ...
  const doctorLicense = req.query.doctorLicense
    ? parseInt(req.query.doctorLicense as string)
    : undefined;
  const patientDni = req.query.patientDni
    ? parseInt(req.query.patientDni as string)
    : undefined;
