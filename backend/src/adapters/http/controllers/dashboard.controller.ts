...
export const getDashboardStats = async (req: Request, res: Response) => {
  ...
  for (const r of rows) {
    ...
    if (!drMap.has(r.doctorLicense)) {
      drMap.set(r.doctorLicense, {
        ...
        doctorName: doc?.user
          ? `${doc.user.lastname}, ${doc.user.name}`
          : `Matrícula ${r.doctorLicense}`,
        specialtyName: sp?.name ?? "-",
        total: 0,
        confirmed: 0,
        pending: 0,
  ...
  const byDoctor: DoctorBucket[] = [...drMap.values()]
    .map((d) => {
      ...
      return {
        ...d,
        absenteeismRate: closed > 0 ? Math.round((d.absent / closed) * 100) : 0,
      };
    })
  ...
  res.json({
    ...
    from,
    to,
    kpis: { total, confirmed, pending, cancelled, absent, absenteeismRate },
    bySpecialty,
    byDoctor,
  });
};
