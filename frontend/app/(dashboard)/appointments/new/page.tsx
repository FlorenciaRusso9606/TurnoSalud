...
export default function NuevoTurnoPage() {
  ...
  useEffect(() => {
    ...
    api.doctors
      .getBySpecialty(Number(specialtyId))
      .then(setDoctors)
      .catch(() => {});
  }, [specialtyId]);
  ...
  useEffect(() => {
    ...
    setFullyBookedDates([]);
    api.availability
      .getByDoctor(Number(doctorLicense))
  ...
  useEffect(() => {
    ...
    api.appointments
      .getAvailable(Number(specialtyId), date)
      .then((all) => {
        const doctorSlots = all.filter(
          (s) => s.doctorLicense === Number(doctorLicense),
        );
        setSlots(doctorSlots);
        const allBooked =
          doctorSlots.length > 0 && doctorSlots.every((s) => s.booked);
        if (allBooked)
  ...
  return (
    <div>
      <PageHeader
        title="Sacar Turno"
        subtitle="Elegí especialidad, médico y horario disponible"
      />
              ...
              {selectedSlot ? (
                <div className="rounded-xl bg-[#2a9d8f]/10 px-3 py-2.5 text-sm text-center">
                  <p className="font-semibold text-[#2a9d8f]">
                    {formatTime(selectedSlot.scheduledAt)} hs
                  </p>
                  <p className="text-xs text-gray-500 mt-0.5">{date}</p>
                </div>
              ) : (
                <p className="text-xs text-gray-400 text-center">
                  Elegí un horario del calendario
                </p>
              )}
