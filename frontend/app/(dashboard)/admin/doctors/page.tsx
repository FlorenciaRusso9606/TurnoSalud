...
export default function AdminDoctorsPage() {
  ...
  useEffect(() => {
    Promise.all([api.doctors.getAll(), api.specialties.getAll()])
      .then(([docs, specs]) => {
        setDoctors(docs);
        setFiltered(docs);
  ...
  const handleEdit = async (e: { preventDefault(): void }) => {
    ...
    setError("");
    try {
      await api.doctors.update(editing.licenseNumber, {
        specialtyId: Number(editSpecialtyId),
      });
      await refresh();
          ...
          <form onSubmit={handleEdit} className="flex items-end gap-4">
            ...
            <div className="flex gap-2 pb-0.5">
              ...
              <Button
                ...
                variant="ghost"
                onClick={closeForm}
                className="text-sm px-4 py-2.5 text-gray-500"
              >
                Cancelar
              </Button>
            </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-[#1d3557]">
                ...
                {!d.isActive && (
                  <span className="ml-2 text-xs bg-red-50 text-red-400 px-1.5 py-0.5 rounded">
                    Inactivo
                  </span>
                )}
              </p>
