...
export default function AdminPatientsPage() {
  ...
  const openCreate = () => {
    setDni(''); setName(''); setLastname(''); setBirthDate(''); setPassword('')
    setSocialWork(''); setEmail(''); setPhone(''); setAddress('')
    setError(''); setSuccess(''); setEditing(null); setMode('create')
  }

  const openEdit = (p: Patient) => {
    setSocialWork(p.socialWork ?? ''); setEmail(p.email ?? '')
    setPhone(p.phone ?? ''); setAddress(p.address ?? '')
    setError(''); setSuccess(''); setEditing(p); setMode('edit')
  }
  ...
  const handleCreate = async (e: { preventDefault(): void }) => {
    ...
    try {
      await api.patients.create({
        dni: Number(dni), name, lastname, birthDate, password,
  ...
  return (
    <div className="max-w-4xl">
      ...
      {mode === 'create' && (
        <AdminFormPanel title="Nuevo paciente" onClose={closeForm} error={error}>
          <form onSubmit={handleCreate} className="grid grid-cols-2 gap-4">
            ...
            <FormInput label="Fecha de nacimiento *" type="date" value={birthDate} onChange={e => setBirthDate(e.target.value)} required />
            <FormInput label="Contraseña *"  type="password" value={password}   onChange={e => setPassword(e.target.value)}   required showToggle />
            <FormInput label="Obra social"   type="text"     value={socialWork} onChange={e => setSocialWork(e.target.value)} />
            <FormInput label="Email"         type="email"    value={email}      onChange={e => setEmail(e.target.value)}      />
            <FormInput label="Teléfono"      type="text"     value={phone}      onChange={e => setPhone(e.target.value)}      />
            <div className="col-span-2">
              <FormInput label="Dirección" type="text" value={address} onChange={e => setAddress(e.target.value)} />
            </div>
        ...
        {filtered.map(p => (
          <div key={p.dni} className="flex items-center gap-3 px-5 py-3.5">
            <div className="w-8 h-8 rounded-full bg-[#e6f5f3] flex items-center justify-center shrink-0">
              <span className="text-xs font-semibold text-[#2a9d8f]">{p.name[0]}{p.lastname[0]}</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-[#1d3557]">{p.lastname}, {p.name}</p>
              <p className="text-xs text-gray-400">
                DNI: {p.dni}
                {p.socialWork && <span className="ml-2 text-[#2a9d8f]">{p.socialWork}</span>}
                {p.phone && <span className="ml-2">{p.phone}</span>}
                {p.email && <span className="ml-2">{p.email}</span>}
              </p>
            </div>
            <Button variant="outline" onClick={() => openEdit(p)} className="flex items-center gap-1 text-xs px-2.5 py-1.5 shrink-0">
              <Pencil size={11} />
              Editar
            </Button>
          </div>
